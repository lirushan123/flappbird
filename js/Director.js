...
import { Tool} from '../js/e.extra.js';
...
export class Director{
    ...
    //判断小鸟与某一个水管是否碰撞(小鸟模型，某个水管的模型)
    isStrike(bird,pipe){
        let strike=true;//假设撞上了
        if(bird.right<pipe.left//小鸟右边小于水管左边
            ||bird.bottom<pipe.top//小鸟下边小于水管上边
            ||bird.left>pipe.right //小鸟左边大于水管右边
            ||bird.top>pipe.bottom//小鸟上边大于水管底部
            ){
                //没撞上
                strike=false;
    ...
    //点击屏幕，小鸟向上飞一段距离
    birdsUp(){
        const birds=this.dataStore.get('birds');//获取变量池中小鸟对象
        for(let i=0;i<3;i++){
